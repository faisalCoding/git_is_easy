<p align="center"><img src="https://github.com/faisalCoding/git_is_easy/blob/main/gitlogo.jpg?raw=true" width="600" alt="git Logo"></p>

## 🗺️ خارطة المقالة
 - لماذا كتبت هذا المقال ❓
 -  ماهو `git`
 -  🖼️ ماهو ال `commit`
 - 🥘 وصفة طبخ ال `commit`
   - 🍴 معدات الطبخة
   - 🥛 مكونات الطبخة
   - المراحل التي يمر بها ال `commit`
 - تعريف مختصر لمجلدات و ملفات `.git`





## ما هو الشيء الذي اوصلك الى هذا المقال او الشيء الذي جعلني اكتب هذا المقال ؟

‏*في الحقيقة الاسطر البرمجية التي يكتبها المبرمج لإنشاء برنامج ما هي المحرك الرئيسي لبرنامج وهي الثمرة التي تحمل القيمة التي نريد الحفاظ عليها وتطويرها بشكل صحيح كي يعمل البرنامج واي خلل بسيط في هذه الاسطر قد تسبب شلل للبرنامج يفقد قيمته*

من الطبيعي أن تحصل هناك أخطاء أثناء كتابة الاسطر البرمجية والتي تسبب بطبيعة الحال إلى توقف البرنامج بشكل كامل أو بشكل جزئي وهذا ما يجعلنا نعيد كتابة الاسطر أو جزء منها او قد نحذفها بشكل كامل
أثناء تلك المحاولات و قد نحتاج إلى اعادة إستخدام اسطر برمجية قد حذفنا سابقا
‏وليس فقط الامر متعلق بالمشاكل التي قد تحدث لبرنامج أثناء تطويره ولكن هناك حالات أخرى مثل إضافة ميزة جديدة أو تحسين كود ما ويعاد كتابة بشكل أفضل

### 🤔 السؤال هنا ؟
كيف يمكننا العودة الى كود قمنا بحذفه او ملف ما مع الحفاظ على الاكواد الحاليه؟
هنا ياتي دور `git`


> [!IMPORTANT]
> وعلى طبيعة الحال فإن هذه الاسطر البرمجية (`content`) تكتب داخل ملفات (`file`) وهذه الملفات لابد أن تكون داخل مجلد (`folder`). من البديهي انك تعلم هذه المعلومات لكن يجب عليك ان تستحضرها في ذهنك 


‏






## ما هو Git ؟ 

هو برنامج يعمل على تتبع مراحل تطوير البرامج و التغيرات التي تحدث اثناء عملية التطوير مثل كتابة أسطر برمجية (`content`) جديدة في ملفات (`file`) المشروع او اضافة ملف (`file`) جديد لمجلد المشروع (`folder`) او حذفه و يقوم بنسخ كل ملفات المشروع في مرحلة معينة اثناء التطوير وضغطها  و ارشفتها في مجلد المستودع و يكون هذا المجلد باسم `.git` ويوجد داخل ملف المشروع الرئيسي



## كيف يعمل؟
**بقوم git بتتبع ملفات المشروع وهذا التتبع له اربعة حالات**

 - اضافة ملف جديد او مجلد<sup>`<-- يحتوي في داخله ملف`</sup>  
 - حذف ملف موجدود او مجلد<sup>`<-- يحتوي في داخله ملف`</sup>  
 - تغيير اسم الملف او مجلد<sup>`<-- يحتوي في داخله ملف`</sup>  
 - تغيير في محتوى الملف ( حذف محتوى / إضافة محتوى )

ـ<sup>`git لا يهتم بالمجلدات الفارغة ولا يقوم بتتبعها الا اذا كانت تحتوي على ملف ما`</sup> 


في حال قمت باي تغيير في مشروعك حسب هذه التغييرات سوف يستشعر git هذا التغيير

ثم بعد ذلك يمكنك بعمل صورة (نسخة) للمشروع في حالته الحالية وهو ما يسمى بـ`commit` في git
 ثم يمكنك بعد ذلك اكمال عملية التطوير و عمل `commit` آخر  مع ضمان الرجوع الى تلك النقاط الـ`commits` التي قمنا بعملها سابقا 

يتعامل git مع مشروعك على انه مجموعة من الـ `objects` بانواع مختلفة تمثل ال(`contents, files, folders`) وهذه النواع هي 

```
object/
  ├── blob ---> # (files) and (content)s this files 
  ├── tree ---> # main project (folder) or subfolders(trees) -> has files(blobs) and subfolders(trees)
  │     ├── tree for -> main project folder
  │     └── tree for -> subfolders
  ├── commit -> # snapshot for the main project folder tree
  └── tag -> # naming of commet
```
 



### لنفهم اكثر ونبدأ بالتطبيق العملي 👨‍💻 انشئ مجلد جديد في حاسوبك لنقوم بإنشاء المجلدات المشاريع البرمجية الخاصة بنا في داخله وبالتلي سنقوم بعمل مستودع خاص بكل مشروع  بعد ذلك نقوم بالدخول الي المجلد مثلا :-
```
mkdir my_projects && cd my_projects
```

### ٣. الان ننشئ مشروعنا الاول و نقوم بعمل مستودع git خاص به

 فلنفترض انه لدينا مشروع لبرمجة نظام اداري لمستشفى ما، سنقوم بعمل مجلد ولنسميه مثلا  `hospital_manegment`
```
mkdir hospital_manegment
```

 ‏هذا المجلد سيكون المجلد الرئيسي للمشروع `main project (folder)` والذي سوف يتتبع git كل الملفات `(files)` و المجلدات `(folders)` حسب
[الحالات الأربع التي ذكرناها سابقا](#كيف-يعمل)

 ‏ولكن قبل كل شيء دعونا نعرف git أن هذا مجلد مشروع نريد ننشئ مستودع خاص به 
  - سوف ندخل الى مجلد المشروع الرئيسي `main project (folder)`
    - ثم ننفذ اول امر لنا في git وهو امر `git init`
```
cd hospital_manegment
git init
```
### 🎉 صار لديك مستودع جاهز لمشروعك

ستجد في داخل مجلد المشروع مجلد جديد تم انشاؤه بعد ما قمنا بتفيذ امر `git init` 
اسم المجلد `.git`


هذا الامر يقوم بعمل مستودع للمشروع في المجلد الذي قمت بتنفيذ هذا الامر فيه و بالتالي سيعتبر git هذا المجلد على انه المجلد الرئيسي للمشروع و اي تغير في هذا المجلد سوف يستشعره git
وسوف تلاحظ انه تم انشاء ملف `.git` و في هذاالملف ستكون فيه النسخ الاحتياطية


## دعنا ناخذ نظرة داخل هذا الملف 
```
cd .git
ls -la
```
ستظهر لنا الجلدات و المفات التالية 
```
.git/
  ├── HEAD
  ├── config
  ├── description
  ├── index
  ├── objects/ 
  ├── refs/
  ├── hooks/
  └── info/
```

سنشرح وظيفة كل ملف و مجلد باختصار لاحقا لكن هل لاحظت مجلد `objects/` ؟ مر معنا سابقا هذا الاسم ! نعم صحيح هو نفس ما تحدثنا عنه 
هنا `git` يقوم بحفظ جميع ال `objects` بكل انواها ( `blobs` `trees` `tag` `commits` ) وهذا المجلد بمثابة `database` لبرنامج git

‏ إذا قمنا بفتح مجلد  `objects/` سيظهر لنا التالي

```
.git/
└── objects/  
    │
    ├── info/            
    └── pack/ 

```
‏ولاحظ أنه لا يوجد سوى مجلدين فارغين و هذا يدل على انه الى هذه اللحظة لا يوجد اي نسخ احتياطية للمشروع وان قاعدة البيانات فارغة تماما 
دعنا الان نقوم بعمل `commit` لنجعل هذا المجلد غير فارغ
اولا نحتاج ان نعرف مكونات ال `commit` يتكون ال `commit` من :-
- tree   <sup> <---- for main project folder</sup>
- parent commit
- author
- commiter
- massages

بالنسبة للمكون الاول ساوضحها بالتشجير التالي
```
commit
  └──  tree [main project folder] 
        ├── blob           [file]
        │
        ├── blob           [file]
        │
        ├── tree           [subfolder]
        │     │
        │     └── blob     [file]
        │
        └── tree           [subfolder]
             │
             ├── blob      [file]
             │
             └── tree      [subfolder] 
                  └── blob [file]

```

اذا لنقوم بعمل مكون  `commit` الاول  tree [main project folder] 

```
git write-tree
```
هذا الامر سيقوم بعمل `tree`  لمجلد المشروع الرئيسي
لكن حاليا مجلد المشروع فارغ تماما لذا لا داعي لعمل هذا الامر حاليا 
لذلك سنقوم بعمل ملف فليكن مثلا `home_page.html` لمشروعنا ونكتب فيه بعض المحتوى

```
echo "the home page" >> home_page.html 
```
لعمل (`blob`) من ملف `home_page.html` 

ـ<sup>`-w تعني كلمة write اي قم بكتابة هذا الـblob داخل قاعدة البيانات database وهي ملف الـ.git/bjects/ `</sup>

```
git hash-object -w home_page.html
```
الان اصبح لدينا `blob` ولو ذهبنا الى  مجلد `objects/` سنشاهد التالي 


```
.git/
└── objects/
    ├── 01/  
    │   └── 825dc642cb6eb9a060e54bf8d69288fbee4904
    ├── d6/              
    │   └── 8407e1c70b25755ad193576311a3780a83cc43 
    ├── info/            
    └── pack/ 

```
‏أصبح لدينا مجلدين داخل كل مجلد ملف اسم هذه الملفات مكتوب بأداة sha1 وهي أداة تشفير عن طريق هذه الشفرة اتعرف git على ال Object
‏
واحد من هذه ال`objects` هو ال ` tree [main project folder] `  الفارغة التي انشأناها سابقا والآخر هو ال `blob` الخاص بملف `home_page.html`  









```
echo -e "blob 15\0im content file" | sha1sum
```
- امر echo يقوم بطباعة نص ما
- خيار -e يعني قم باعتبار الـ empty-byte `00` كما هو مكتوب في الامر اعلاه `\0`
- علامة `|` تعني القيمة التي سيتم ارجاعها في الامر السابق ادخله كمتغير في الامر التالي `sha1sum`
- اداة sha1sum تقموم بعمل تشفير من نوع sha1
 

```
git hash-object -w home_page.html
```


```
git hash-object -w home_page.html
```


```
git hash-object -w home_page.html
```





